                         ...
                         )
{
    ...
    for (auto i = 0U; i <= segment_count; ++i) {
        float next_x{std::cosf(angle + angle_delta) * radius1};
        float next_y{std::sinf(angle + angle_delta) * radius1};
        float next_u{0.5f + std::cosf(angle + angle_delta) * 0.5f * radius1norm};
        float next_v{1.0f - (0.5f + std::sinf(angle + angle_delta) * 0.5f * radius1norm)};
    ...
    angle = 0.0f;
    for (auto i = 0U; i <= segment_count; ++i) {
        float next_x{std::cosf(angle + angle_delta) * radius2};
        float next_y{std::sinf(angle + angle_delta) * radius2};
